...
public:
    ...
    struct Config
    {
        ...
        // Default constructor
        Config()
            : bind_address("127.0.0.1"), port(2106), max_connections(500), rsa_key_pool_size(10), enable_logging(true), connection_timeout(300)
        {
        }
    };
...
public:
    explicit LoginServer(const Config &config = Config{});
    ~LoginServer();
...
extern LoginServer *g_login_server_instance;