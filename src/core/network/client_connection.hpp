...
// Callback types for packet creation (to decouple from login-specific implementations)
using PacketCreator = std::function<std::shared_ptr<ReadablePacket>(const std::vector<uint8_t> &data, const ScrambledRSAKeyPair &rsa_pair)>;
using InitPacketCreator = std::function<std::vector<uint8_t>(int32_t session_id, const ScrambledRSAKeyPair &rsa_pair, const std::vector<uint8_t> &blowfish_key)>;
...
private:
    ...
    // Packet creation callbacks (to decouple from login-specific implementations)
    PacketCreator packet_creator_;
    InitPacketCreator init_packet_creator_;
...
public:
    ...
    // Packet creation callback setup (allows login module to provide implementations)
    void set_packet_creator(PacketCreator creator) { packet_creator_ = std::move(creator); }
    void set_init_packet_creator(InitPacketCreator creator) { init_packet_creator_ = std::move(creator); }
