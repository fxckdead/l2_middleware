                                        ...
                                        const std::vector<uint8_t> &blowfish_key)
{
    ...
    try
    {
        ...
        // Use callback to create init packet (allows login module to provide the implementation)
        if (init_packet_creator_)
        {
            auto packet_data = init_packet_creator_(session_id_, rsa_pair, blowfish_key);
            if (!packet_data.empty())
            {
                send_init_packet_raw(packet_data);
                set_state(State::INIT_SENT);
                log_connection_event("Init packet sent via callback");
            }
            else
            {
                log_connection_event("Init packet creator returned empty data");
                force_disconnect();
            }
        }
        else
        {
            log_connection_event("No init packet creator configured");
            force_disconnect();
        }
    }
    catch (const std::exception &e)
...
void ClientConnection::handle_complete_packet(std::vector<uint8_t> packet_data)
{
    try
    {
        ...
        // Use callback for packet creation (allows login module to provide the implementation)
        if (packet_creator_ && rsa_pair_)
        {
            auto packet = packet_creator_(packet_data, *rsa_pair_);
        ...
        {
            log_connection_event("No packet creator or RSA key pair available");
        }
    }
    catch (const std::exception &e)
    {
