...
// Callback types for connection management
using ConnectionPacketHandler = std::function<void(std::shared_ptr<ReadablePacket>, ClientConnection *)>;
using InitPacketHandler = std::function<void(std::shared_ptr<ClientConnection>, const ScrambledRSAKeyPair &, const std::vector<uint8_t> &)>;
...
private:
    ...
    // Callback handlers (to decouple from login-specific implementations)
    ConnectionPacketHandler packet_handler_;
    InitPacketHandler init_packet_handler_;
...
public:
    ...
    // Callback setup (allows login module to provide implementations)
    void set_packet_handler(ConnectionPacketHandler handler) { packet_handler_ = std::move(handler); }
    void set_init_packet_handler(InitPacketHandler handler) { init_packet_handler_ = std::move(handler); }
